...
export class CMesh extends BABYLON.Mesh {
    ...
    createClone(item: any, useInstances: any, name: any) {
        var c;
...
export class ColorEffector {
    ...
    animate(index, frame) {
        return ColorEffector.getColor(
            this._reverse ? index : 1 - index,
            frame,
            this._autoanimate ? (frame * this._framerate) / Math.PI : 0
        );
    }
    static getColor(index: number, frame: number, offset: number = 0) {
        ...
        let ping = Math.abs(index - fx) > 1 / 140 ? 0.99 : 0;
...
export class Cloner {
    ...
    ePosition(vec: any): BABYLON.Vector3 {
        var vRet = Cloner.vZero.add(vec);
        for (let i = 0; i < this._effectors.length; i++) {
            vRet = BABYLON.Vector3.Lerp(
                vec,
...
export class RadialCloner extends Cloner {
    ...
    private _colorize: ColorEffector | undefined;
...
export class LinearCloner extends Cloner {
    ...
    calcPos2() {
        ...
        //shift offset
        this._clones[0].position = BABYLON.Vector3.Lerp(
            Cloner.vZero,
            this._P,
            f * this._offset
        );
...
//
export class RainbowPalette {
    static getColor(index: any, frame: any) {
        let pi2 = Math.PI;
        let off = Math.sin(frame * 0.01);
        let aa = Math.sin((index + 0.05 + off) * pi2);
        let bb = Math.sin((index + 0.34 + off) * pi2);
...
export class ColorEffector {
    ...
    static smoothstep(min: any, max: any, value: any) {
        var x = Math.max(0, Math.min(1, (value - min) / (max - min)));
        return x * x * (3 - 2 * x);
    }
    constructor() {
